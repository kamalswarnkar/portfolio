...
export const AlienMotifs = memo(function AlienMotifs({ active = true }: { active?: boolean }) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden opacity-95">
      {/* Fire motif — shown on all devices */}
      <motion.div
      ...
      {/* Scan line group — shown on all devices */}
      <motion.div
      ...
      {/* Crystal shards — desktop only (too many animations on mobile) */}
      {!isMobile && (
        <motion.div
        ...
        >
          {["left-6 top-10", "right-10 top-20", "left-16 bottom-10"].map((slot, index) => (
            <motion.div
              key={slot}
